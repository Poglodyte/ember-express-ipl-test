...
function manholeCreate(name, lat, lng, cb) {
  manholedetail = { 
    name: name,
    location: { type: "Point", coordinates: [ lat, lng] }
  }
    
  var manhole = new Manhole(manholedetail);    
  manhole.save(function (err) {
    if (err) {
      cb(err, null)
      return
    }
    console.log('New Manhole: ' + manhole);
    cb(null, manhole)
  }  );
}


function createManholes(cb) {
  async.parallel([
      function(callback) {
        manholeCreate('manhole_1', -70.001, 40, callback);
      },
      function(callback) {
        manholeCreate('manhole_2', -70.002, 40, callback);
      }
    ],
    // Optional callback
    cb);
...
db.manholes.insert( {
  name: "AnotherManhole",
  location: { type: "Point", coordinates: [ -70.012, 40 ] }
});

db.manholes.createIndex( { location: "2dsphere" } )
